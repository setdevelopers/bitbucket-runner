...
RUN apk add busybox=1.35.0-r18 && \
    ...
    apk add openjdk17-jdk=17.0.10_p7-r0 && \
    rm -rf /var/cache/apk/*
...
# Install Bitbucket Runner
ARG BITBUCKET_RUNNER_VERSION="3.10.0"
    
RUN mkdir atlassian-bitbucket-pipelines-runner && \
    curl --silent --proto '=https' --tlsv1.2 -fOL https://product-downloads.atlassian.com/software/bitbucket/pipelines/atlassian-bitbucket-pipelines-runner-${BITBUCKET_RUNNER_VERSION}.tar.gz && \
    tar -xzf atlassian-bitbucket-pipelines-runner-${BITBUCKET_RUNNER_VERSION}.tar.gz -C atlassian-bitbucket-pipelines-runner